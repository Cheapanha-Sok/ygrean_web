...
export default function Account() {
  const { dispatch, user, loading } = useContext(UserDataContext);
  ...
  const handleEdit = async(e)=>{
    e.preventDefault()
    const {name , isGraduate} = inputData
    const res = await editUser(user.id , name , isGraduate)
          ...
          <div className="flex flex-col gap-2 md:gap-5">
            ...
            <div className="flex flex-col">
              ...
              {isEdit ? (
                ...
                <span>{user.isGraduate ? "Graduate" : "Under Graduate"}</span>
              )}
            </div>
            {isEdit && (
